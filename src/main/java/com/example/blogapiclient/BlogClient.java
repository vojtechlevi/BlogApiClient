...
public class BlogClient {
    ...
    // Vår konstruktor
    public BlogClient(String apiAddress) {
    ...
    public boolean addPost(BlogPost newPost) {
    ...
    public BlogPost[] printListOfPosts() {
        BlogPost[] blogPosts = {};

        String target = "/list";

        System.out.println("Getting Posts from " + apiAddress + target);
    ...
    public boolean updatePostById(int id, BlogPost updatePost){
        ...
        try {
            ...
            try (OutputStream os = connection.getOutputStream()) {
                ...
                byte[] input = updatePost.toJson().getBytes(StandardCharsets.UTF_8);
                os.write(input, 0, input.length);
            }
    ...
    public boolean clearPosts() {
        String target = "/clear"; // http://127.0.0.1:8080/api/v1/post/clear
